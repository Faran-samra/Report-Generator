...
const CosmeticGrade = ({ phoneData, updatePhoneData, nextStep, prevStep }) => {
  ...
  const handleGradeSelect = (grade) => {
    ...
    const basePrice = phoneData.diagnosticResults?.estimatedValue || 300;
    const finalPrice = Math.round(basePrice * grade.priceMultiplier);
    updatePhoneData({ 
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">
                    Price Impact: {grade.priceMultiplier === 1.0 ? 'No reduction' : `${Math.round((1 - grade.priceMultiplier) * 100)}% reduction`}
                  </span>
                  
                  {phoneData.diagnosticResults && (
                    <span className={`font-semibold ${grade.textColor}`}>
                      ${Math.round((phoneData.diagnosticResults.estimatedValue || 300) * grade.priceMultiplier)}
                    </span>
                  )}
                </div>
              </div>
